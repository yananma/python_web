...
列表，列表就是动态数组  
```
Python 中所有的对象都存放在双向链表中，列表也是对象，所以列表也存放在双向链表中。所以有 prev 和 next 两个指针  
ob_refcnt 引用计数器  
**ob_item 指针的指针，用来存放列表元素；指针放的是地址  
ob_size 列表中元素的个数  
allocated 列表容量，最多可以容纳多少元素；会有扩容和缩容  

创建空列表：op->ob_item = NULL; 和 Py_SET_SIZE(op, size)  

添加元素：先判断容量，容量够就添加，不够就开辟内存空间；
开辟内存空间，如果有后面有足够的连续空间，就会在后面开辟空间，如果没有，就找一个新的地方开辟一块空间； 
PyList_SET_ITEM(self, n, v); 将元素添加到列表的 ob_item 中。  
list_resize 是改变空间大小的函数，有一个判断，如果有空间，而且空间占用率大于容量的一半，就往后添加；
如果没有空间就扩容，如果空间占用率低就缩容，减少空间浪费。  
开辟内存的时候，一次开辟一段内存，不够再开辟新的。内存是动态变化的。  

插入元素：insert(0, value)，之前的指针都要后移，最开始的指针指向新插入的值。  
for (i = n; --i >= where; )
    items[i+1] = items[i];    # 插入位置之后的所有元素全部后移
items[where] = v;    # 空出来的位置插入新的元素  

移除元素：pop，引用数减 1，内存内容都还保留，下次 append 的时候，直接覆盖就行了。因为删除非常麻烦，这样最简单。  

清空元素：clear，所有的内容都置空，Py_SET_SIZE(a, 0); a->ob_item = NULL; a->allocated = 0;，内存内容还是保留，下次覆盖。  

销毁列表：del，先判断引用计数，如果还有别的引用，那就只是引用次数减 1，没有别的操作；
如果都没有引用了，就会和上面清空元素一样，都置空；
并不会释放内存，而是会把空列表放到缓存 free_list 中，缓存可以存放 80 条列表。
这样再次创建新的列表的时候，直接去缓存中拿就行了，省了很多麻烦。  
```

元组，元组就是静态列表  
```
Python 中所有的对象都存放在双向链表中，元组也是对象，所以元组也存放在双向链表中。所以有 prev 和 next 两个指针  
ob_refcnt 引用计数器  
*ob_item[1] 指针类型的数组，并且只能存放一个元素，用来存放元组中的元素  
ob_size 列表中元素的个数  

比起列表，没有 allocated，元组创建以后就不能再改变了，不能 append，不能 pop。这一条是可变对象和不可变对象的核心差异。  

创建元组：tuple_new_impl 就是 __new__ 方法的底层实现；
如果是空元组，return tuple_get_empty(); 创建空元组，就会开辟了一个空间  
如果是非空元祖，return PySequence_Tuple(iterable);  
创建非空元组，PySequence_Tuple() 分三种情况：
第一，如果格式为 v1 = tuple(1, 2, 3) 或 v2 = tuple((1, 2, 3))，即传入的就是元组，会进行 PyTuple_CheckExact(v)，
然后检查通过的话就直接 return v。而且 v2 是指向 v1 元素的，并不会创建新的元组。
第二，如果传入的是列表，比如 tuple([1, 2, 3])，就会执行 PyList_AsTuple()，会循环列表中的每一个元素，然后把地址 copy 过来，放到元组中。
第三，如果传入的是迭代器，比如 tuple(range(10))，不知道元素个数是多少个，因为是一个一个生成的，所以就会先猜一个。
猜对了就获取值，放到元组中去，猜小了就扩容，扩容是按照特定的规定，跃阶性增加，猜大了就缩容。  

元组切片的时候，只是 copy 了结构体对象，内容没有改变。  

元组也有自己的缓存机制，删除的时候会放大缓存中，如果又创建了元素个数相同的
```
