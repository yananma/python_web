...
class HotTopicsViewSet(ViewSet):
        ...
        similarity_data = [
            {
                'source':topic_a_name,
                'target':topic_b_name,
                'lineStyle':{'width':sim*50}
            }
            for topic_a_name,topic_b_name,sim in TopicSimResult.objects.filter(task=task_info).values_list('topic_a__name','topic_b__name','sim_num')
        ]
        ...
        results['children']=[
            {
                ...
                'value':TopicModel.objects.filter(id=topic_id).values_list('name',flat=True),
                'children':list(
    ...
    def river(self,request:Request):
        ...
        results.update({
            ...
            'data':list(TopicYearCountResult.objects.filter(topic_id__in=ids).values_list('year','count','topic__name'))
        })
        return Response(results)
...
class TopicViewSet(ViewSet):
    ...
    def topic_list(self, request):
        task_info: TaskInfo = request.task_info
        return Response({
            'data':list(TopicModel.objects.filter(task_id=task_info.taskid).order_by('sort').values('id','name','sort','keywords','task_id'))
        })
